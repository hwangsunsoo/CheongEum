...
# 훈맹청음 개발본 | 訓盲聽音 開發本
스크린리더를 통해 들리는 소리가 화면에 나타난 정보와 달라 의미가 서로 통하지 아니하므로 (문제 원인)<br>
시력이 낮은 사람이 이루고자 하는 바가 있어도 그 뜻을 펴지 못하는 경우가 많습니다. (문제 제시)<br>
이를 안타깝게 여겨 새롭게 280여줄의 코드를 작성하여 웹 브라우저에서 정보를 음성으로 변환하는 기능을 만들었으니 (연구 요약)<br>
사용자가 정보에 자유롭게 접근하기에 편안하게 하고자 할 따름입니다. (기대 효과)

## 🎯 개요
'훈맹청음'은 웹 브라우저의 음성 합성 기능을 활용하여 텍스트를 음성으로 변환하는 자바스크립트 플러그인입니다.<br>
2025년 디지털 교과서 개발 과정에서 보편적 학습 설계(Universal Design for Learning, UDL)의 적용 필요성에 의해 개발되었습니다.<br>
빠르게 변화하는 웹 환경 속에서 스크린리더의 업데이트 속도는 이를 따라가지 못하는 경우가 많습니다.<br>
웹 개발자들은 다양한 브라우저와 스크린리더 조합을 모두 테스트하는 것이 현실적으로 어렵기 때문에, 특정 조합에서만 접근성이 확보되는 경우가 빈번합니다.<br>
이로 인해 시력이 낮거나 문해력이 낮은 사용자들이 최신 웹 기술과 콘텐츠를 원활히 활용하지 못하는 문제가 발생하고 있습니다.<br>
훈맹청음은 이러한 문제를 해소하고자 개발되었습니다. 이 플러그인은 웹 접근성을 향상시키고, 모든 사용자가 웹 콘텐츠에 쉽고 자유롭게 접근할 수 있도록 지원하는 것이 목표입니다.<br>
훈맹청음의 적용을 통해 웹 개발자는 더 이상 접근성 테스트의 복잡성으로 고민할 필요 없이, 모든 사용자를 고려한 웹 환경을 보다 쉽게 구현할 수 있습니다.<br>
앞으로도 훈맹청음은 웹 환경의 변화에 발맞추어 지속적인 업데이트와 개선을 통해 모든 사용자가 차별 없이 웹 콘텐츠를 이용할 수 있도록 노력할 것입니다.

## 🌟 명칭
​'훈맹청음'이라는 이름은 깊은 의미와 한국적인 정체성을 담고 있어 웹 기반 스크린리더의 이름으로 매우 적합합니다.<br>
...
## 📢 마무리
'훈맹청음'은 웹 접근성을 높이고, 다양한 사용자에게 웹 콘텐츠를 보다 쉽게 전달할 수 있는 자바스크립트 플러그인입니다.<br>
웹 개발자라면 이 플러그인을 활용하여 보다 포용적인 웹 환경을 구축해보세요.<br>
...
Universal Design for Learning, 보편적 학습 설계는 다양한 학습자의 요구와 능력을 고려하여 모든 학습자가 효과적으로 학습할 수 있도록 지원하는 교육 프레임워크입니다.<br>
...
## 🌟 UDL의 필요성
전통적인 교육 방식은 평균적인 학습자를 기준으로 설계되어, 다양한 학습자의 요구를 충분히 반영하지 못하는 경우가 많습니다.<br>
UDL은 이러한 한계를 극복하고자 모든 학습자가 동등하게 학습에 참여할 수 있는 환경을 조성합니다.<br>
...
`SpeechSynthesisUtterance`는 웹 브라우저에서 텍스트를 음성으로 변환하는 기능을 제공하는 Web Speech API의 핵심 인터페이스입니다.<br>
이를 활용하면 웹 페이지에서 사용자에게 음성 피드백을 제공하거나, 접근성을 향상시키는 등 다양한 기능을 구현할 수 있습니다.<br>
브라우저의 음성 합성 기능을 제어하는 객체로, 말할 텍스트와 음성의 속성(언어, 속도, 높낮이 등)을 설정할 수 있습니다.<br>
...
`SpeechSynthesisUtterance`를 활용하면 웹 애플리케이션에 음성 기능을 손쉽게 통합할 수 있습니다.<br>
